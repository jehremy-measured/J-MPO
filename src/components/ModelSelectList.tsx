...
export function ModelSelectList({ selectedModelId, onSelect, verb = "created" }: Props) {
  if (isOnLatestModel) {
    return (
      <p className={styles.modelPlainText}>
        Plan will be {verb} using data from the latest MIM update ({LATEST_MODEL.date}).
      </p>
    );
  }
        ...
        <label className={`${styles.modelRow} ${selectedModelId === "current" ? styles.modelRowSelected : ""}`}>
          ...
          <span className={styles.modelBody}>
            ...
            <span className={styles.modelSubtext}>
              Plan will be {verb} using the same MIM data ({CURRENT_MODEL.date}) as your current plan.
            </span>
          </span>
        </label>
