...
import styles from "./PlanSalesChart.module.css";
...
export function PlanSalesChart({ planStart, planEnd, totalSales, totalBudget }: Props) {
  const gradientId = useId();
  const [hoverIndex, setHoverIndex] = useState<number | null>(null);

  const weeks = useMemo(
    () => buildWeeklyProjection(planStart, planEnd, totalSales, totalBudget),
    [planStart, planEnd, totalSales, totalBudget]
  );
      ...
      <div className={styles.card}>
        <header className={styles.header}>
          <div className={styles.headerText}>
            <span className={styles.title}>Projected weekly performance</span>
            <span className={styles.subtitle}>Incremental sales and budget pacing across the plan period.</span>
          </div>
